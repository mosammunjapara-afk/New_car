"""
debug_jeep.py — Jeep India ka price structure dhoondo
======================================================
Jeep India: Compass, Meridian, Wrangler, Grand Cherokee.
Ye script site ka structure dekhta hai — model/price links, variant API, prices.

CHALAO:
    python debug_jeep.py
Phir jeep_debug.txt UPLOAD kar dena.
"""

from playwright.sync_api import sync_playwright
import re, json

BASE = "https://www.jeep-india.com"
...
def real_prices(body):
    return sorted(set(int(n) for n in re.findall(r'\b(\d{6,8})\b', body) if 1000000 <= int(n) <= 9999999))
...
with sync_playwright() as p:
    ...
    def on_response(resp):
        ...
        if "json" in ct:
            try:
                ...
                return
            if real_prices(b) and any(k in b.lower() for k in ["variant","price","version","grade","model","trim"]):
                apis.append((u, b))
    ...
    try:
        ...
        models = ["compass","meridian","wrangler","grand-cherokee","price"]
    ...
    targets = [h for h in mlinks if any(k in h.lower() for k in ["price","compass","meridian"])][:4]
    if not targets:
        targets = [f"{BASE}/compass.html", f"{BASE}/meridian.html"]
        ...
        try:
            ...
            if plines:
                ...
                for i, l in plines[:15]:
                    ...
                    log(f"    [{prev[:22]}] -> {l[:38]}")
            for trim in ["Sport","Longitude","Limited","Model S","Night Eagle","Overland","80th Anniversary","Trailhawk"]:
...
with open("jeep_debug.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(out))

print("\n\n" + "=" * 60)
print("HO GAYA. jeep_debug.txt UPLOAD kar do.")
print("=" * 60)