"""
debug_landrover.py — Land Rover India ka price structure dhoondo
=================================================================
Land Rover India: Range Rover, RR Sport, RR Velar, RR Evoque, Defender,
Discovery, Discovery Sport.
Ye script site structure dekhta hai.

CHALAO:
    python debug_landrover.py
Phir landrover_debug.txt UPLOAD kar dena.
"""
...
with sync_playwright() as p:
    ...
    def on_response(resp):
        ...
        if "json" in ct:
            try:
                ...
                return
            if real_prices(b) and any(k in b.lower() for k in ["variant","price","model","derivative","vehicle"]):
                apis.append((u, b))
    page.on("response", on_response)

    log("Land Rover homepage...")
    try:
        ...
        kws = ["price","range-rover","defender","discovery","velar","evoque","models","vehicles"]
    ...
    targets = [h for h in mlinks if any(k in h.lower() for k in ["price","range-rover","defender"])][:3]
...
with open("landrover_debug.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(out))

print("\n\n" + "=" * 60)
print("HO GAYA. landrover_debug.txt UPLOAD kar do.")
print("=" * 60)