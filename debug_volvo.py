"""
debug_volvo.py — Volvo India ka price structure dhoondo
========================================================
Volvo India: XC40, XC60, XC90, S90, C40 Recharge, EX40, EX90 (EV).
Ye script site structure dekhta hai.

CHALAO:
    python debug_volvo.py
Phir volvo_debug.txt UPLOAD kar dena.
"""

from playwright.sync_api import sync_playwright
import re, json

BASE = "https://www.volvocars.com/in"
...
with sync_playwright() as p:
    ...
    def on_response(resp):
        ...
        if "json" in ct:
            try:
                ...
                return
            if real_prices(b) and any(k in b.lower() for k in ["variant","price","model","carline","vehicle"]):
                apis.append((u, b))
    ...
    try:
        ...
        kws = ["price","cars","xc40","xc60","xc90","s90","c40","ex40","ex90","models"]
    ...
    targets = [h for h in mlinks if any(k in h.lower() for k in ["price","xc60","xc90","cars"])][:3]
    if not targets and mlinks:
        targets = mlinks[:3]
        ...
        try:
            ...
            if apis:
                ...
                for u, b in apis:
                    ...
                    names = re.findall(r'"(?:name|model|carline|title)"\s*:\s*"([^"]{2,45})"', b)
                    if names:
            ...
            if plines:
                ...
                for i, l in plines[:20]:
                    ...
                    log(f"    {' | '.join(x[:20] for x in ctx)}")
        except Exception as e:
            log(f"  fail: {str(e)[:50]}")

    browser.close()

with open("volvo_debug.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(out))

print("\n\n" + "=" * 60)
print("HO GAYA. volvo_debug.txt UPLOAD kar do.")
print("=" * 60)